...
func (s *Server) logout(req *LogoutRequest) *LogoutResponse {
...
func (s *Server) canRead(userID, orgID string) bool {
	return s.Data.Org(orgID).CanRead(userID)
}

func (s *Server) canWrite(userID, orgID string) bool {
...
func (s *Server) createUser(phone util.PhoneNumber) (*User, error) {
	u := &User{
		ID:    util.RandomID(),
		Phone: phone,
	}
